create or replace function public.heymies_meta_text_array(meta jsonb, key text)
returns text[]
language sql
immutable
as $$
  select case
    when jsonb_typeof(meta -> key) = 'array' then
      coalesce(
        array(
          select nullif(trim(value), '')
          from jsonb_array_elements_text(meta -> key) as value
          where nullif(trim(value), '') is not null
        ),
        '{}'::text[]
      )
    when jsonb_typeof(meta -> key) = 'string' and nullif(trim(meta ->> key), '') is not null then
      array[trim(meta ->> key)]
    else '{}'::text[]
  end;
$$;
...
where public.email_preferences.ctid = ranked_preferences.ctid
